...
class Service {
  ...
  load(rawOptions) {
    ...
    ]).then(args => that.loadServiceFileParam(...args, options));
  }

  loadServiceFileParam(serviceFilename, serverlessFileParam, options) {
    const that = this;
    ...
    if (_.isObject(serverlessFile.service)) {
      ...
      if (options.service) that.serviceObject.name = options.service;
      that.service = options.service || serverlessFile.service.name;
    } else {
      that.serviceObject = { name: options.service || serverlessFile.service };
      that.service = options.service || serverlessFile.service;
    }
