...
describe('Service', () => {
  ...
  describe('#load()', () => {
    ...
    it('should load serverless.yml from filesystem and set service name from options', () => {
      const SUtils = new Utils();
      const serverlessYml = {
        service: 'new-service',
        provider: {
      ...
      return expect(
        serviceInstance.load({ service: 'options-service-name' })
      ).to.eventually.be.fulfilled.then(() => {
        expect(serviceInstance.service).to.be.equal('options-service-name');
        expect(serviceInstance.provider.name).to.deep.equal('aws');
        expect(serviceInstance.provider.variableSyntax).to.equal(
          '\\${{([ ~:a-zA-Z0-9._@\'",\\-\\/\\(\\)*]+?)}}'
        );
